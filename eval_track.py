...
def compute_dense_embeddings(tracks, track_lengths, encoder, fusion_layer, num_targets=4, num_points=10):
    """
    Compute embeddings for each timestep t (from t to end)
    ...
    for b in range(batch_size):
        ...
        for t in range(actual_len):
            # Extract segment from t to end
            # track_segment = tracks[b, t:actual_len]  # (future_len, num_targets, num_points, 3)
            track_segment = tracks[b, 0:t+1]  # (future_len, num_targets, num_points, 3)

            future_len = track_segment.shape[0]
            
            # Reshape for encoder: (num_targets, future_len, num_points, 3)
...
def visualize_similarity_matrix(embeddings, save_path, title_suffix=""):
    """
...
def visualize_target_similarities(embeddings, save_path, title_suffix=""):
    """
    ...
    for target_idx in range(num_targets):
        ...
        ax.set_title(f'Target {target_idx} Temporal Similarity', fontsize=12, fontweight='bold')
...
def evaluate(args):
    ...
    with torch.no_grad():
        for batch_idx, data in enumerate(tqdm(dataloader)):
            ...
            # tracks = data['robot_tracks_rel'].to(device)
            # lengths = data['robot_track_lengths'].to(device)
            ...
            batch_embeddings = compute_dense_embeddings(
                ...
                args.num_targets, args.num_points
            )
            ...
            if batch_idx > args.num_vis_samples:
                for i, embeds in enumerate(batch_embeddings):
                    ...
                    visualize_similarity_matrix(embeds, fig_path, 
                                               f'\nSample {sample_idx}')
                    
                    # Visualize per-target temporal similarity
                    fig_path = os.path.join(args.output_dir, 
                                           f'target_similarity_sample_{sample_idx:04d}.png')
                    visualize_target_similarities(embeds, fig_path,
                                                 f'\nSample {sample_idx}')
