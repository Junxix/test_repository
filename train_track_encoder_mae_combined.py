...
def train_mae(args_override):
    ...
    for epoch in range(args.resume_epoch + 1, args.num_epochs):
        ...
        for data in pbar:
            ...
            robot_tracks = data['robot_tracks_rel'].to(device)
            robot_lengths = data['robot_track_lengths'].to(device)

            batch_size = human_tracks.shape[0]
            human_T = human_tracks.shape[1]
            batch_size, robot_T, N, _ = robot_tracks.shape
            ...
            robot_tracks = robot_tracks.view(batch_size, robot_T, args.num_targets, args.num_points, 3)
            ...
            for target_idx in range(args.num_targets):
                target_tracks = human_tracks[:, :, target_idx, :, :]
                loss, _, _ = model(target_tracks, human_lengths)
                total_loss += loss
            
            # Process robot tracks
            for target_idx in range(args.num_targets):
                target_tracks = robot_tracks[:, :, target_idx, :, :]
                loss, _, _ = model(target_tracks, robot_lengths)
                total_loss += loss
