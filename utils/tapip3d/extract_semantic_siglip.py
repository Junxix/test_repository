...
# import numpy as np
# from transformers import SiglipModel, SiglipImageProcessor  # 改这里
...
# processor = SiglipImageProcessor.from_pretrained(model_path)  # 改这里
...
# # 参数设置
# patch_size = 14  # 根据你选择的模型调整
# image_size = 384  # SigLIP-SO400M 使用 384，可根据模型调整
# grid_size = image_size // patch_size  # 27 for 384
...
#     pixel_values = inputs['pixel_values']  # [1, 3, image_size, image_size]
    
#     # 提取特征
#     with torch.no_grad():
#         outputs = vision_model(pixel_values=pixel_values, output_hidden_states=True)
#         # SigLIP 的输出结构与 CLIP 相似
#         # outputs.last_hidden_state: [1, num_patches, hidden_dim] (SigLIP 没有 CLS token)
#         features = outputs.last_hidden_state
    
#     # 提取 patch tokens (SigLIP 默认没有 CLS token，直接使用即可)
#     patch_tokens = features.squeeze(0)  # [num_patches, hidden_dim]
#     # patch_tokens = F.normalize(patch_tokens, p=2, dim=1)  # 如果需要归一化
    
#     feature_dim = patch_tokens.shape[-1]  # SigLIP-SO400M 是 1152
