...
from policy.minkowski.resnet import ResNet14Max


class Sparse3DEncoder(torch.nn.Module):
    def __init__(self, input_dim=256, output_dim=512):
        """
        Modified encoder using ResNet14Max (same as SpatialAligner.conv).
        Input is precomputed sinput features (256-dim after interp_proj).
        """
        super().__init__()
        self.input_dim = input_dim
        self.output_dim = output_dim
        # Use ResNet14Max with same config as SpatialAligner
        self.cloud_encoder = ResNet14Max(
